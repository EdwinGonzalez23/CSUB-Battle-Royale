...
void init_opengl()
{
    ...
    glGenTextures(1, &gl.ayylmao);
    int w = img[0].width;
    int h = img[0].height;
    glBindTexture(GL_TEXTURE_2D, gl.ayylmao);
...
void render()
{
    ...
    if (gl.credits){
	show_credits();
	extern void andrew_credits_picture(int x, int y, GLuint texid);
	andrew_credits_picture(200, 200, gl.ayylmao);
    } else{
